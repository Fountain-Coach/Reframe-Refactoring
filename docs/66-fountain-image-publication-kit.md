...
1. Paste an image into the active Fountain surface, choose an image from the system pasteboard, or ask the Copilot to
   generate one.
2. Reframe creates an intake record and uploads the source to Image Cloud; the writer is never asked to manage a local
   image folder.
3. Reframe shows the image inline with its current state: private, rights incomplete, generated, review required,
   ready, or published.
4. The writer may edit, caption, credit, or ask the Writing Coach for help. Every transformation creates a new lineage.
...
Pasting is an intake gesture, not publication consent. Private preview is never silently promoted to public delivery.
...
- pasteboard or explicit selection creates `ImageIntakeRequest`;
- the Image Cloud client uploads and returns a durable cloud reference;
- the Fountain editor renders the reference as an inline visual projection;
