...
4. **What will not parse is marked, never smoothed.** A stretch the rules cannot type is returned as
...
5. **Hard prose is the test, not the exception.** Ulysses stops obeying by *Proteus* and abandons the rules in
...
6. **Fixed weights are versioned.** Tooling assets (embedding revisions, tagger models) can change between OS
...
7. **The model's share shrinks as the parse grows,** and that is the goal. Every fact moved down a tier makes the
...
- Every projection has tests over **unmodified source text** of the work it claims to handle, including at least
  one stretch that legitimately returns `unclassified`.
