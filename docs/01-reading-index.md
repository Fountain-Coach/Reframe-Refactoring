...
| [Copilot Reading Surface and Typography](45-copilot-reading-surface-and-typography.md) | Copilot belongs to Reframe's beat and uncertainty workspace: situated, legible and Reframe-native, not a website sidebar. |
...
| [One Definition, Two Projections](49-one-definition-two-projections.md) | An operation is defined ONCE in the IDL — where QoS, capability masks, budgets, correlation and chunking can be said — and projected twice: onto a backplane topic and onto an OpenAPI-described HTTP route, both generated, never authored. The web does not speak MIDI 2.0; that is a reason to project, not to define twice. A service binds by publishing a document. |
